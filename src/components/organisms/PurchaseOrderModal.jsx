...
const PurchaseOrderModal = ({ isOpen, onClose, purchaseOrder, onSave }) => {
  ...
  useEffect(() => {
    if (purchaseOrder) {
      ...
      setOrderItems(purchaseOrder.items || []);
    } else {
      setFormData({
        supplierId: "",
        status: "draft",
  ...
  const handleAddItem = () => {
    ...
    if (existingItemIndex >= 0) {
      ...
      updatedItems[existingItemIndex].total = updatedItems[existingItemIndex].quantity * newItem.unitCost;
      ...
      const item = {
        ...
        total: parseInt(newItem.quantity) * parseFloat(newItem.unitCost)
      };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const poData = {
        ...
        orderDate: formData.status === 'ordered' && !purchaseOrder?.orderDate 
  ...
  const productOptions = products.map(product => ({
    ...
    label: `${product.name} - Current Stock: ${product.stockLevel}`
  }));
          ...
          <div className="border-t pt-6">
            ...
            {orderItems.length > 0 ? (
              ...
              <div className="text-center py-8 text-gray-500">
                No items added yet. Add items to the purchase order above.
              </div>
            )}
          </div>
